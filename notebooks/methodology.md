...
4. Validated the resulting medians against public press releases to ensure plausibility, then exported the curated payloads (`data/derived_summary.json` and `data/viz_payload.json`). The front-end now hydrates these files via `js/modules/data.js`, eliminating the legacy embedded-data shim while keeping offline-ready fallbacks in notebooks.
